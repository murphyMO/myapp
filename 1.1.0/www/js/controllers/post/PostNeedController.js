 ...
 * 发布需求控制器
 */
app.controller('PostNeedController',
	['$scope', '$rootScope', '$localStorage', '$ionicHistory','$state', '$ionicActionSheet', '$CommonFactory', '$cordovaCamera', '$ionicModal', '$ionicScrollDelegate', '$timeout', '$ionicLoading', 'TransferPostDataService', 'CommunityService', 'PostService', 'BusinessService',
	function ($scope, $rootScope, $localStorage, $ionicHistory, $state, $ionicActionSheet, $CommonFactory, $cordovaCamera, $ionicModal, $ionicScrollDelegate, $timeout, $ionicLoading, TransferPostDataService, CommunityService, PostService, BusinessService) {
		...
		$scope.postNeed = function() {
			...
			if (!($scope.article.title && $scope.article.title.length > 0)) {
				$CommonFactory.showAlert("请填写需求标题");
				return;
			}
			if (!($scope.article.content && $scope.article.content.length > 0)) {
				$CommonFactory.showAlert("请填写需求内容");
				return;
			}
			...
			PostService.postNeed(data, function(res){
