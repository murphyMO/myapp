 ...
 * 预订活动场地控制器
 */
app.controller('SpaceRoomListController',
	...
	function ($scope, $rootScope, $state,$stateParams, $localStorage,BusinessService,$ionicScrollDelegate, $CommonFactory) {
			...
			if (!$scope.isRun) {
				...
				BusinessService.getConferenceList(data,function(response){
					...
					for(var i = 0;i < response.data.length;i++){
						...
						// 非会员价
						response.data[i].unit_price_ary = response.data[i].unit_price.split("#")
						response.data[i].fourHoursAgoPrice = response.data[i].unit_price_ary[0];
						response.data[i].fourHoursAfterPrice = response.data[i].unit_price_ary[1];
						response.data[i].min_price = response.data[i].unit_price_ary.sort(function(a,b){
							return a-b
						})[0]
						//会员价
						response.data[i].unit_price_member_ary = response.data[i].unit_price_member.split("#")
						response.data[i].fourHoursAgoPriceMember = response.data[i].unit_price_member_ary[0];
						response.data[i].fourHoursAfterPriceMember = response.data[i].unit_price_member_ary[1];
						response.data[i].min_price_member = response.data[i].unit_price_member_ary.sort(function(a,b){
							return a-b
						})[0]
		...
		$scope.spaceDetail = function(i,from){
			if($rootScope.isGuest){
				$state.go("login");
				// $CommonFactory.showAlert("大侠您好，该功能需登录查看");
			}else{
			$localStorage.setObject("space_item",i);
			$state.go("spaceRoomDetail",{'from': from});
			}
		}
