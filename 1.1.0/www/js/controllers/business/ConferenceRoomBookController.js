 ...
 * 预订会议室控制器
 */
app.controller('ConferenceRoomBookController',
	...
	function ($scope, $rootScope, $state,$stateParams, $localStorage,$CommonFactory,BusinessService,$timeout,$ionicModal) {
		...
		$scope.conferenceroom.people_number = $scope.item.attendee_amount
		...
		$scope.getDisabledConference = function(){
			var data = {
		...
		$scope.$watch("times",function(newVal,oldVal){
			if (newVal != oldVal) {
				...
				if ($scope.user.user_type <= '1') {
					//非会员总价格
					$scope.total = $scope.item.unit_price * 10000 * $scope.userCheckTimes/10000;
				}
				else{
					//会员总价格
					$scope.total = $scope.item.unit_price_member * 10000 * $scope.userCheckTimes/10000;
		...
		$scope.create = function(){
			if(!$scope.conferenceroom.people_number){
				$CommonFactory.showAlert("请填写参会人数");
				return;
			}
			...
			var data = {
				...
				comments : $scope.conferenceroom.comments,
				attendee_amount : $scope.conferenceroom.people_number,
		...
		$scope.topBack = function(){
			$localStorage.set("people_number","")
			window.history.back();
		}
	}
]);
