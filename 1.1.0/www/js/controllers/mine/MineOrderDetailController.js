 ...
 * 单个订单详情-控制器
 */
app.controller('MineOrderDetailCtrl',
	['$scope', '$rootScope', 'MineOrdersService','$stateParams','$state','$ionicPopup','$CommonFactory','$localStorage','$cordovaToast','$ionicModal',
	function($scope, $rootScope, MineOrdersService, $stateParams,$state,$ionicPopup,$CommonFactory,$localStorage,$cordovaToast,$ionicModal) {
		...
		$scope.getOrderDetail = function(){
			...
			MineOrdersService.orderDetail(data,function(response){
				if (response.statuscode == 1) {
					...
					$scope.mineOrderListItem = $localStorage.getObject('mineOrderListItem');
					$scope.mineOrderListItem.status_text = $scope.order.order_info.status_text;
					$scope.mineOrderListItem.status = $scope.order.order_info.status;
					$localStorage.setObject('mineOrderListItem',$scope.mineOrderListItem);
				}
			});
		}
		// 加载数据
		...
		$scope.goToOver = function(){
			...
			MineOrdersService.changeStatus(data,function(response){
