 ...
 * 企业未支付列表-控制器
 */
app.controller('BalanceListCtrl',
	...
	function ($scope, $rootScope, $state,$localStorage, $CommonFactory,$ionicModal,CompanyBillService) {
		...
		$scope.doRefresh = function(){
			...
			$scope.items = [];
			$scope.currentPage = 1;
			$scope.getCompanyBalanceList();
			$scope.$broadcast('scroll.refreshComplete');
		}

		$scope.$on('$ionicView.beforeEnter',function (){
			$scope.unPay = $localStorage.getObject('unPay');
		})
		...
		//未支付列表
		$scope.getCompanyBalanceList = function(){
			if (!$scope.isRun) {
				$scope.isRun = true;
				var data = {
		...
		// 触发一个按钮点击，或一些其他目标
		$ionicModal.fromTemplateUrl('templates/modal/balance_message_modal.html', {
			scope: $scope,
