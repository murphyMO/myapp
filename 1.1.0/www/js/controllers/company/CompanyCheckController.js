 ...
 * 加入企业审核申请-控制器
 */
app.controller('CompanyCheckCtrl',
	...
	$scope.updateAptitude = function(){
		if($scope.change==1){
			$CommonFactory.showAlert("未提交任何信息！");
			return ;
		}
		if(!$scope.comObj.com_name){
		...
		if(!$scope.comObj.com_legal_representative_mobile){
			$CommonFactory.showAlert("请填写法人联系电话!");
			return;
		}
		...
		if ($scope.comObj.com_legal_representative_mobile) {
			var owner_phoneReg = /^1[3|4|5|7|8]\d{9}$/ || /^(\(\d{3,4}\)|\d{3,4}-|\s)?\d{7,14}$/ ;
			if (!owner_phoneReg.test($scope.comObj.com_legal_representative_mobile)) {
			$CommonFactory.showAlert("法人联系方式格式不正确！");
			return;
			}
		}
		if($scope.comObj.com_establish_time) {
		...
		var data = {
			...
			'com_legal_representative_mobile' : $scope.comObj.com_legal_representative_mobile,
		...
		CompanyService.applychenck(data,function(response){
			if(response.statuscode==CODE_SUCCESS){
				$CommonFactory.showAlert("信息已提交，小师妹审核中");
				$state.go("comadd");
			}
		})
	}
